    ...
    for (unsigned _k=0; _k<k_cross; ++_k) {
        ...
        for (unsigned __itr_all=0; __itr_all<max_itr_all; ++__itr_all) {
            ...
            std::cout << "cross-validation:" << _k+1 << " itr_all:" << __itr_all+1 <<
                         " cur_loss:" <<cur_loss <<" time" << (t2-t1)/CLOCKS_PER_SEC <<std::endl;
        }
        ...
        auto tk2 = clock();
        std::cout << _k+1 << "th validation, accuracy:" << std::setprecision(10) << _accy_k <<
                     " time:" << (tk2-tk1)/CLOCKS_PER_SEC << std::endl;
    }
}
