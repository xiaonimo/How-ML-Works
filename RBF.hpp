...
class rbf {
public:
    rbf(points_t& _train_x, points_t& _train_y, std::initializer_list<unsigned> _layers,
        unsigned _n_epoch=100, unsigned _n_batch=5, double _min_loss=0.1, double _learning_rate=0.01, bool _verbose=true):

        train_x(_train_x), train_y(_train_y), layers(_layers), n_input(layers[0]), n_hidden(layers[1]), n_output(layers[2]),
        n_epoch(_n_epoch), n_batch(_n_batch), n_step(train_x.size()/n_batch), min_loss(_min_loss), learning_rate(_learning_rate),
        verbose(_verbose){
...
private:
    ...
    std::vector<vec_t> batch_dw, batch_dc;
    vec_t batch_db, batch_dr;

public:
    index_t x_index, y_index;
    points_t &train_x, &train_y;
    std::vector<unsigned> layers;
    const unsigned n_input, n_hidden, n_output;
...
rbf::predict(const points_t &test_x) {
    ...
    for (auto _o:_y) res.push_back(nn::argmax(_o));
    return res;
}
